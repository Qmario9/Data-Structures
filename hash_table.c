...
int hash_function1(struct hash_table* hash_table, char* key) {
  printf("%4d / %4d = %4d\n", key[0], hash_table->size, ( (int) key[0] ) % hash_table->size) ;
...
int hash_function2(struct hash_table* hash_table, char* key) {
  ...
  //printf("(%4d + %4d) / %4d = %4d\n", key[0], key[1], sizeof(key), (key[0]+key[1])/sizeof(key));
  
  return (key[0]+key[1])/(sizeof(key)*length);
}
...
int hash_table_collisions(struct hash_table* hash_table) {
  ...
  for(int i = 0; i < 8; i++){
    struct node * idk = hash_table->array[i];
    if(idk != NULL){
      idk = idk->next;
      while(idk != NULL){
        idk = idk->next;
